        ...
        if (playable_colors[props.game.active_player] == props.color) {
          if (props.game.board_active) {
            ...
            if(props.color === 'yellow'){
              var linked = new URL(window.location.href);
              var playerId = linked.searchParams.get("user");
              var gameCoins = linked.searchParams.get("gamecoins");
              fetch('https://admin.spl.games/user/multigame/'+playerId+'/'+gameCoins+'/1/ludo').then(()=>{
                console.log('player:'+playerId+", looses ludo. And lost:"+gameCoins+"coins");
              });
            }else{
              var linked = new URL(window.location.href);
              var playerId = linked.searchParams.get("user");
              var gameCoins = linked.searchParams.get("gamecoins");
              fetch('https://admin.spl.games/user/multigame/'+playerId+'/'+gameCoins+'/0/ludo').then(()=>{
                console.log('player:'+playerId+", looses ludo. And lost:"+gameCoins+"coins");
              })
  ...
  _createClass(Game, [{
    ...
    value: function initGame() {
      ...
      this.setState({
        game: Object.assign({}, game, {
          game_state: 2,
          players:[
            {color:"green",type:"HUMAN"},
            {color:"yellow",type:"HUMAN"},
          ],
          active_player: 0
        })
      });
    ...
    value: function rollDice(player_idx) {
      ...
      if(dice_roll === 6){
        this.setState({
          game: Object.assign({}, game, {
           sixTerm:this.state.game.sixTerm+1
          })
        });
        console.log(this.state.game.sixTerm);
      }else{
        this.setState({
          game: Object.assign({}, game, {
           sixTerm:0
